...
class purchase_order_line(orm.Model):
    ...
    def _get_date_planned(self, cr, uid, seller, start_date, context=None):
        """This method overload the method _get_date_planned and use the
            method get_date to consider the working days, and change the start
            date in function of the supplier_shortage
        """
        if seller and seller.supplier_shortage:
            start_date = seller.supplier_shortage
        return super(purchase_order_line, self)._get_date_planned(
            cr, uid, seller, start_date, context=context)
